...
@
     ...
     \begin{block}{Identification}
       \justifying
       \secintro{Running search engines in \R and parsing identification files.}
       \Rpackage{mzID}, (\Rpackage{mzR}), \Rpackage{MSnID}, 
       \Rpackage{rTANDEM}, (\Rpackage{MSGFplus})

     \end{block}

      \begin{block}{Quantitation}
        \justifying
        \secintro{Quantitative proteomcs in \R.}
        \Rpackage{MSnbase}, \Rpackage{MALDIquant}, \Rpackage{synapter}
      \end{block}
    \end{column}
